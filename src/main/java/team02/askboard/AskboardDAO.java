...
public class AskboardDAO extends OracleDB {
	...
	public void insertAsk(AskboardDTO dto) throws Exception{
		...
		/*try {
			conn = getConnection();
			pstmt = conn.prepareStatement("select id, pw from member; ");			
	...
	public List getAsk(int start, int end) throws Exception {
		...
		try {
			...
			pstmt = conn.prepareStatement(
					...
					+ " (select * from askboard order by ref desc, re_step asc ) b) "
					+ " where r >= ? and r <= ? ");
	...
	public String getWriter(int num) throws Exception{
		...
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement("select count(*) from askboard where ref=(select ref from askboard where num=?)and re_step = 0");
				pstmt.setInt(1,num);
			rs = pstmt.executeQuery();
			if (rs.next()) {
